...
for filename in os.listdir(path='unsorted4'):
    # create dataframe from csv
    print(f'\n\ncurrent file: {filename}')
    csv_df = pd.read_csv(filepath_or_buffer=f'unsorted4\{filename}', encoding='ISO-8859-1')
    ...
    if filename == 'Gallery_Collection-The_Gallery_Collect_Google_Merchant_Feed-shopping.csv':
        image_links = image_links[1800:]
    ...
    for index, link in image_links.items():
        ...
        try:
            if verify_image_link(url=link):
                ...
                try:
                    with open(file='invalids\invalid_images_4.txt', mode='+a') as txt_file:
    ...
    os.remove(path=f'unsorted4\{filename}')