...
for filename in os.listdir(path='unsorted2'):
    # create dataframe from csv
    print(f'\n\ncurrent file: {filename}')
    csv_df = pd.read_csv(filepath_or_buffer=f'unsorted2\{filename}', encoding='ISO-8859-1')
    ...
    if filename == 'Full_Compass_Systems-Product_Feed-shopping.csv':
        image_links = image_links[50000:]
    ...
    for index, link in image_links.items():
        ...
        try:
            if verify_image_link(url=link):
                ...
                try:
                    with open(file='invalids\invalid_images_2.txt', mode='+a') as txt_file:
    ...
    os.remove(path=f'unsorted2\{filename}')